...
exit Nvim (impossible!!), at which point it will merge this instance's changes
with the on-disk database, so that multiple running instances do not clobber
each other's arglists.
