...
function Health.check()
    ...
    if vim.uv.fs_access(db_path, "r") then
        ...
        if #content > 0 then
            ...
            if m_res then
                ...
                error("DB is corrupted: invalid msgpack.", ("Delete the corrupted file: `rm %q`"):format(db_path))
            end
        else
            info("DB is empty.")
        end
    ...
    if has_mini_misc then
        ...
        elseif type(mini_misc.setup_restore_cursor) == "function" then
            warn("`setup_restore_cursor()` is inactive.", 'Enable with `require("mini.misc").setup_restore_cursor()`')
        end
    else
