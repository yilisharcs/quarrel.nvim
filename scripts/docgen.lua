    ...
    for _, f in ipairs(fields) do
        ...
        if f.type:match("^" .. spec.id .. "%.") then
            ...
            if sub_block then
                ...
                local sub_lines = H.synthesize_lua_table(doc, sub_fields, indent .. H.sw)
...
H.collect_sections = function(block, options, acc_tbl)
    for _, s in ipairs(block) do
        ...
        for _, line in ipairs(s) do
            local is_invalid = type(line) ~= "string" or line:match("^%s*%-+%s*$") or line:find("Fields ~", 1, true)
...
H.prepare_doc_tree = function(doc, is_readme)
    ...
    if is_readme and blocks.config then
        ...
        -- NOTE: mini.doc indents code blocks on its own. we account for that by
        --       passing the indent value twice.
        local synthesized = H.synthesize_lua_table(doc, cfg_fields, H.sw .. H.sw)
...
readme_hooks.write_pre = function(lines)
    ...
    local res = {
        ...
        H.sw .. '"' .. repo .. '",',
        H.sw .. "init = function()",
        H.sw .. H.sw .. "vim.g." .. spec.id .. " = { --[[ config goes here ]] }",
        H.sw .. "end",
        "}",
        "```",
        "",
